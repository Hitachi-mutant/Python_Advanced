...
with open('Python_Advanced/args_sum_execution_time.txt', "a") as file:
    file.write("'args_sum' function was executed on: ")

def execution_time(func_to_decorate):
    def wrapper(*args):
        start_time = time.time()
        start_time_readable = time.strftime("%d-%m-%Y %H:%M:%S", time.localtime(start_time))
        result = start_time_readable
        print(f"'args_sum' function was executed on: {start_time_readable}")
        with open('Python_Advanced/args_sum_execution_time.txt', "a") as file:
